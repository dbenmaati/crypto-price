          <!-- start paginator -->
            ...
            <div class="pagination-container">
              ...
              <ul class="pagination">
                  ...
                  @else
                  <li>
                    <a href="{{ $exchanges->url($start) }}" tabindex="0" role="button" aria-disabled="false" aria-label="Previous page" rel="prev">
                              <span class="paginate-item">
                                  <i class="fa-solid fa-arrow-left"></i>
                                  <span>&nbsp;&nbsp;PREV</span>
                              </span>
                ...
                <!-- Pagination Middle --> 
                  @if($start > 1)
                      <li class="page">
                          <a role="button" href="{{ $exchanges->url(1) }}" tabindex="0" aria-label="Page {{1}}">{{1}}</a>
                      </li>
                  @endif
                      @for ($i = $start; $i <= $end; $i++)
                        <li class="page {{ ($exchanges->currentPage() == $i) ? ' active' : '' }}">
                            <a role="button" href="{{ $exchanges->url($i) }}" tabindex="0" aria-label="Page {{$i}}">{{$i}}</a>
                        </li>
                      @endfor
                <!-- Show Last Page --> 	
                  @if($end < $exchanges->lastPage())
                    <li class="page">
                        <a role="button" href="{{ $exchanges->url($exchanges->lastPage()) }}" tabindex="0" aria-label="Page {{$exchanges->lastPage()}}">{{$exchanges->lastPage()}}</a>
                    </li>
                  @endif
                <!-- Next Button --> 
                @if ($exchanges->onLastPage())
                  <li class="disabled">
                ...
                @else
                  <li>
                    <a href="{{ $exchanges->url($end) }}" tabindex="0" role="button" aria-disabled="false" aria-label="Next page" rel="next">
                      <span class="paginate-item"> <span>NEXT</span>&nbsp;&nbsp; <i class="fa-solid fa-arrow-right"></i> </span>
                    </a>
                  </li>
                  @endif
