    ...
    <script>
        async function fetchCryptoData() {
            try {
                ...
                // Sort by changePercent24Hr to find top gainers
                const sortedAssets = assets.sort((a, b) => parseFloat(b.changePercent24Hr) - parseFloat(a.changePercent24Hr));
                
                // Take top 5 gainers
                const topGainers = sortedAssets.slice(0, 15);

                // Insert rows into the table
                const tableBody = document.getElementById('crypto-table-body');
                topGainers.forEach(asset => { tableBody.innerHTML += createTableRow(asset); });
            } catch (error) {
        ...
        function createTableRow(asset) {
            return `
                <tr>
                    ...
                    <td class="boild"><a href="/coin/${asset.id}" style="color:#6ccf59;"> <i class="fa-solid fa-caret-up"></i> &nbsp;&nbsp; ${parseFloat(asset.changePercent24Hr).toFixed(2)}%</a></td>
                    <td class="up"><a href="/coin/${asset.id}">$ ${parseFloat(asset.priceUsd).toFixed(2)}</a></td>
